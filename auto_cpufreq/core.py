...
def get_override():
    if os.path.isfile(governor_override_state):
        with open(governor_override_state, "rb") as store: return pickle.load(store)
    else: return "default"
...
try: dist_name = distro.id()
    ...
    else: # This should not be the case. But better be sure.
...
def turbo(value: bool = None):
    ...
    if p_state.exists():
        ...
        if amd_value == "active": print("CPU turbo is controlled by amd-pstate-epp driver")
        return False    # Basically, no other value should exist.
    else:
...
def charging():
    ...
    if len(power_supplies) == 0: return True    # nothing found found, so nothing to check
    else:                                       # we found some power supplies, lets check their state
        for supply in power_supplies:
            ignore_supply = any(item in supply for item in POWER_SUPPLY_IGNORELIST) # Check if supply is in ignore list
            if ignore_supply: continue # If found in ignore list, skip it.
            try:
                with open(Path(power_supply_path + supply + "/type")) as f:
                    supply_type = f.read()[:-1]
                    if supply_type == "Mains":                          # We found an AC
                        try:
                            with open(Path(power_supply_path + supply + "/online")) as f:
                                val = int(f.read()[:-1])
                                if val == 1: return True                # We are definitely charging
                        except FileNotFoundError: continue              # We could not find online, check next item
                    elif supply_type == "Battery":                      # We found a battery, check if its being discharged
                        try:
                            with open(Path(power_supply_path + supply + "/status")) as f:
                                val = str(f.read()[:-1])
                                if val == "Discharging": return False   # We found a discharging battery
                        except FileNotFoundError: continue              # Could not find status, check the next item
                    else: continue                                      # Continue to next item because current is not "Mains" or "Battery"
            except FileNotFoundError: continue                          # Could not find type, check the next item
    return True                                                         # We can't determine discharging state, assume we are on powercable

def get_avail_gov():
    return Path("/sys/devices/system/cpu/cpu0/cpufreq/scaling_available_governors").read_text().strip().split()

def get_avail(reverse:bool):
    """
    Iterate over ALL_GOVERNORS: from performance to powersave (invert if reverse=True)
    :return:
    """
    gov = ALL_GOVERNORS[::-1] if reverse else ALL_GOVERNORS
    for g in gov:
        if g in get_avail_gov(): return g

def get_avail_performance(): get_avail(False)
def get_avail_powersave(): get_avail(True)
...
def cpufreqctl():
    ...
    """
    if IS_SNAP: pass
    else: # deploy cpufreqctl.auto-cpufreq script
...
def cpufreqctl_restore():
    ...
    """
    if IS_SNAP: pass
    elif os.path.isfile("/usr/local/bin/cpufreqctl.auto-cpufreq"):
...
def deploy_complete_msg():
    separator(" auto-cpufreq daemon installed and running ")
    print("To view live stats, run:\nauto-cpufreq --stats")
    print("\nTo disable and remove auto-cpufreq daemon, run:\nsudo auto-cpufreq --remove")
    separator()

def deprecated_log_msg():
    separator(" auto-cpufreq log file renamed ")
    print("The --log flag has been renamed to --stats\n")
    print("To view live stats, run:\nauto-cpufreq --stats")
    separator()

def remove_complete_msg():
    separator(" auto-cpufreq daemon removed ")
    print("auto-cpufreq successfully removed.")
    separator()

def deploy_daemon():
    separator(" Deploying auto-cpufreq as a daemon ")
...
def deploy_daemon_performance():
    separator(" Deploying auto-cpufreq as a daemon (performance) ")
...
def remove_daemon():
    ...
    separator(" Removing auto-cpufreq daemon ")
...
def gov_check():
    for gov in get_avail_gov():
        if gov not in ALL_GOVERNORS:
            separator(" Checking for necessary scaling governors ")
...
def set_frequencies():
    ...
    if hasattr(set_frequencies, "prev_power_supply") and power_supply == set_frequencies.prev_power_supply: return
    ...
    for freq_type in frequency.keys():
        ...
        try:
            frequency[freq_type]["value"] = value if value else int(conf[power_supply][freq_type].strip())
        except ValueError:
...
def display_average_core_temperatures(cpuload):
    print(f"Optimal total CPU usage: {cpuload} %, high average core temp: {avg_all_core_temp} °C")

# set powersave and enable turbo
def set_powersave():
    conf = config.get_config()
    if conf.has_option("battery", "governor"): gov = conf["battery"]["governor"]
    else: gov = get_avail_powersave()
    ...
    if not Path("/sys/devices/system/cpu/cpu0/cpufreq/energy_performance_preference").exists(): print('Not setting EPP (not supported by system)')
    else:
        dynboost_enabled = Path("/sys/devices/system/cpu/intel_pstate/hwp_dynamic_boost").exists()

        if dynboost_enabled:
            dynboost_enabled = bool(int(os.popen("cat /sys/devices/system/cpu/intel_pstate/hwp_dynamic_boost").read()))
    ...
    if auto == "always":
        ...
        if cpuload >= 20: set_turbo(True)
            ...
            display_average_core_temperatures(cpuload)
            set_turbo(False)
...
def set_performance():
    ...
    if Path("/sys/devices/system/cpu/cpu0/cpufreq/energy_performance_preference").exists() is False:
        ...
        if dynboost_enabled:
            dynboost_enabled = int(os.popen("cat /sys/devices/system/cpu/intel_pstate/hwp_dynamic_boost").read())
        if dynboost_enabled: print('Not setting EPP (dynamic boosting is enabled)')
        else:
...
def mon_performance():
    ...
    if psutil.cpu_percent(percpu=False, interval=0.01) >= 20.0 or max(psutil.cpu_percent(percpu=True, interval=0.01)) >= 75:
        print("High CPU load", end="")
    elif load1m > performance_load_threshold: print("High system load", end="")
    else: print("Load optimal", end="")
    ...
    # set turbo state based on average of all core temperatures
        ...
        display_average_core_temperatures(cpuload)
        print("suggesting to set turbo boost: off")
    else: print("suggesting to set turbo boost: on")
...
def is_running(program, argument):
    ...
    for p in psutil.process_iter():
        ...
        for _ in filter(lambda x: program in x, cmd):
            if argument in cmd: return True

def daemon_running_msg():
    separator(" auto-cpufreq running ")
...
def daemon_not_running_msg():
    separator(" auto-cpufreq not running ")
