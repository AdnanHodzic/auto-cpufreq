...
def app_version():
    ...
    # snap package
    if os.getenv("PKG_MARKER") == "SNAP": print(getoutput(r"echo \(Snap\) $SNAP_VERSION"))
    # aur package
    elif dist_name in ["arch", "manjaro", "garuda"]:
        if call("pacman -Qs auto-cpufreq > /dev/null", shell=True) == 1: print(get_formatted_version())
...
def check_for_update():
    ...
    latest_release_url = "https://api.github.com/repos/AdnanHodzic/auto-cpufreq/releases/latest"
    try:
    ...
    if latest_version is not None:
        ...
        if latest_version == installed_version: print("auto-cpufreq is up to date")
        else:
...
def get_formatted_version():
    ...
    formatted_version = splitted_version[0] + " (git: " + splitted_version[1] + ")" if len(splitted_version) > 1 else ''
                    ...
                    if supply_type == "Mains":
                        ...
                        try:
                            with open(Path(power_supply_path + supply + "/status")) as f:
                                if str(f.read()[:-1]) == "Discharging": return False   # we found a discharging battery
...
def cpufreqctl():
    ...
    """
    if not os.path.isfile("/usr/local/bin/cpufreqctl.auto-cpufreq"): # deploy cpufreqctl.auto-cpufreq script if not running on a SNAP
...
def cpufreqctl_restore():
    ...
    if os.path.isfile("/usr/local/bin/cpufreqctl.auto-cpufreq"): # remove cpufreqctl.auto-cpufreq script if not running on a SNAP
